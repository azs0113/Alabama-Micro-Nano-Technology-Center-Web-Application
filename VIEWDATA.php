<!DOCTYPE HTML>
<html xmlns="http://www.w3.org/1999/xhtml">
<html>
<head>
	<meta charset="UTF-8">
	<title>View Data Laboratory Website</title>
	<link rel="stylesheet" href="css/style.css" type="text/css">
	<link href="HCIcss/bootstrap.min.css" rel="stylesheet">
	<style type ="text/css">
    
	.nav-tabs li a {font-size:18px; color:#0E0D21;}
	img {
	width:70px;
	left: 30px;
	height:60px;
	position: absolute;
	float: left;
	top: 15px;
    right: 0;
	}

	
    	</style>
	
</head>
<body>
	<div id="header">
		<a href="index.html" class="logo"><img src="images/auburn.png" align ="left" style= "width:200px;left: 30px; height:80px;" alt=""></a>
		<ul>
			<li class="selected">
				<a href="index.html">Home</a>
			</li>
			<li>
				<a href="ABOUT.html">About Us</a>
			</li>
			<li>
				<a href="LOGINPAGE.html">Add Data</a>
			</li>
			<li>
				<a href="SEARCHDATA.html">Search Data</a>
			</li>
			<li>
				<a href="VIEWDATA.php">View Data</a>
			</li>
		
		</ul>
	</div>
	

 
 
<div class ="container">
        <ul class ="nav nav-tabs" role = "tablist">
        	<li class ="active"><a href="#Books" role ="tab" data-toggle ="tab">Central</a></li>
        	<li><a href="#Customers" role ="tab" data-toggle ="tab">Ion Data</a></li>
        	<li><a href="#Employees" role ="tab" data-toggle ="tab">Ebeam Data</a></li>
        	<li><a href="#Orders" role ="tab" data-toggle ="tab">Sputter Data</a></li>
        </ul>
                
                <div class ="tab-content">
                	<div class ="tab-pane face active in" id ="Books">
                    
                    	<div class = "container">
                        	<table class="table">
                            
				<?php

					$dbhost = "";
					$dbuser = "";
					$dbpass = "";
					$dbname = "";
					$tables = array('db_main','db_ion', 'db_ebeam', 'db_sputter');
					if (!mysql_connect($dbhost, $dbuser, $dbpass))
        					die("Can't connect to database");

					if (!mysql_select_db($dbname))
        					die("Can't select database");


					$result = mysql_query("SELECT * FROM $tables[0]");


					echo "<thead>";
					echo "<tr>";

					$heads = mysql_num_fields($result);
					for($i=0; $i<$heads; $i++)
					     {
        					$field = mysql_fetch_field($result);
        					echo "<th>{$field->name}</th>";
					     }
						
					echo "</tr>";
					echo "</thead>";

					while ($row = mysql_fetch_row($result))
					     {
        					echo "<tr>";
        					foreach($row as $cell)
                					echo "<td>$cell</td>";
        						echo "</tr>";
					     }
					mysql_free_result($result);

				 ?>

                               
                                </table>
                           </div>  
			   <div class="row">
				<div class="col-lg-4">			
				   </div>
					<div class="col-lg-8">
					  <div id="result">
				          </div>
			                </div>
		                </div>
                           </div>
                    <div class = "tab-pane fade" id ="Customers">
                    <div class = "container">
                    <table class="table">
                            
                    <?php

					$dbhost = "";
					$dbuser = "";
					$dbpass = "";
					$dbname = "";
					$tables = array('db_main','db_ion', 'db_ebeam', 'db_sputter');
					if (!mysql_connect($dbhost, $dbuser, $dbpass))
        					die("Can't connect to database");

					if (!mysql_select_db($dbname))
        					die("Can't select database");
				$result = mysql_query("SELECT * FROM $tables[1]");

				echo "<thead>";
				echo "<tr>";
				$heads = mysql_num_fields($result);
				for($i=0; $i<$heads; $i++)
					{
        					$field = mysql_fetch_field($result);
        					echo "<th>{$field->name}</th>";
					}

				echo "</tr>";
				echo "</thead>";
				while ($row = mysql_fetch_row($result))
					{
        					echo "<tr>";
        					foreach($row as $cell)
                				echo "<td>$cell</td>";
        					echo "</tr>";
					}
				mysql_free_result($result);
		    ?>
                               
                    </table>
                 </div>
                 </div>
                 <div class = "tab-pane fade" id ="Employees">
                    <div class = "container">
                        <table class="table">
                                	<?php

					$dbhost = "";
					$dbuser = "";
					$dbpass = "";
					$dbname = "";
					$tables = array('db_main','db_ion', 'db_ebeam', 'db_sputter');
						if (!mysql_connect($dbhost, $dbuser, $dbpass))
        						die("Can't connect to database");

						if (!mysql_select_db($dbname))
        						die("Can't select database");


						$result = mysql_query("SELECT * FROM $tables[2]");
						echo "<thead>";
						echo "<tr>";

						$heads = mysql_num_fields($result);
						for($i=0; $i<$heads; $i++)
							{
        							$field = mysql_fetch_field($result);
        							echo "<th>{$field->name}</th>";
							}

						echo "</tr>";
						echo "</thead>";

						while ($row = mysql_fetch_row($result))
							{
        						echo "<tr>";
        						foreach($row as $cell)
                						echo "<td>$cell</td>";
        							echo "</tr>";
							}
						mysql_free_result($result);

					?>
                               
                            </table>
							
                        </div>
                    </div>
                    <div class = "tab-pane fade" id ="Orders">
                    	<div class = "container">
                        	<table class="table">
                            
                                	<?php

					$dbhost = "";
					$dbuser = "";
					$dbpass = "";
					$dbname = "";
					$tables = array('db_main','db_ion', 'db_ebeam', 'db_sputter');
						if (!mysql_connect($dbhost, $dbuser, $dbpass))
        						die("Can't connect to database");

						if (!mysql_select_db($dbname))
        						die("Can't select database");

						$result = mysql_query("SELECT * FROM $tables[3]");


						echo "<thead>";
						echo "<tr>";

						$heads = mysql_num_fields($result);
						for($i=0; $i<$heads; $i++)
							{
        							$field = mysql_fetch_field($result);
        							echo "<th>{$field->name}</th>";
							}
						echo "</tr>";
						echo "</thead>";

						while ($row = mysql_fetch_row($result))
							{
        						echo "<tr>";
        						foreach($row as $cell)
                					echo "<td>$cell</td>";
        						echo "</tr>";
							}
						mysql_free_result($result);

					?>
                               
 
                               
                            </table>
                        </div>
                    </div>
					
					
	
		
		<button type=button class="btn btn-default" onClick="location.href='LOGINPAGE.html'">Login to add new Data</button>
	

						
        	</div> 
 
	<script src="http://ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js"></script>
    
    <script src="bootstrap.js"></script>
    
</body>
</html>
